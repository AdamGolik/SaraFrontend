...
export const auth = {
  ...
  register: async (email: string, password: string, name: string, lastname: string) => {
    const response = await api.post('/register', { email, password, name, lastname });
    return response.data;
  },
};
...
export const clients = {
  getAll: async (page = 1, perPage = 10, search?: string) => {
    try {
      console.log('Fetching clients list with params:', { page, perPage, search });
      const params = { page, per_page: perPage, search };
      const response = await api.get('/clients/', { params });
      console.log('Clients list response:', response.data);
      // Log actual client UUIDs
      if (response.data.clients && response.data.clients.length > 0) {
        console.log('Available client UUIDs:', response.data.clients.map((client: any) => client.uuid));
      }
      return response.data;
    } catch (error: any) {
  ...
  getByDateRange: async (from: string, to: string) => {
    try {
      console.log('Fetching clients by date range:', { from, to });
      const params = { from, to };
      const response = await api.get('/clients/', { params });
      console.log('Date range response:', response.data);
      return response.data.clients;
    } catch (error: any) {
      console.error('Error fetching clients by date range:', {
        from,
        to,
        status: error.response?.status,
        data: error.response?.data,
